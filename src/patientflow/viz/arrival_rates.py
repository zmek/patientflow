...
from patientflow.calculate import calculate_time_varying_arrival_rates_lagged, process_arrival_rates, get_true_demand_by_hour
...
def annotate_hour_line(
    ...
    if slope is not None and x1 is not None:
        ...
        plt.plot([0 - x_margin, a], [y_a, y_a], color='grey', linestyle=line_styles[hour_line])
        annotation_text = f'{annotation_prefix}, {int(y_a)} beds needed by {hour_line}:00'
        y_position = y_a + 1
    else:
        plt.annotate(
            '',
        ...
        annotation_text = f'{annotation_prefix}, {int(y_value)} beds needed by {hour_line}:00'
...
def plot_arrival_rates(inpatient_arrivals,
        title,
        lagged_by=None,
    ...
    arrival_rates_dict = calculate_time_varying_arrival_rates(inpatient_arrivals, time_interval)
    ...
    if arrival_rates_spread is not None and len(arrival_rates_spread) > 0:
        # Base arrival rates
        plt.plot(x_values, y_values, marker='x', color='grey', markersize=4, 
                linestyle=':', linewidth=1, label='Arrival rates of admitted patients')
        
        # Lagged arrival rates
        plt.plot(x_values, get_cyclic_data(arrival_rates_lagged), marker='o', markersize=4, 
                color='grey', linestyle='--', linewidth=1, 
                label='Average number of beds needed 4 hours after arrival')
        
        # Spread arrival rates
        plt.plot(x_values, get_cyclic_data(arrival_rates_spread), marker='o',
        ...
        # Lagged arrival rates
        plt.plot(x_values, get_cyclic_data(arrival_rates_lagged), marker='o', color='C0',
                label='Average number of beds needed 4 hours after arrival')
...
def plot_cumulative_arrival_rates(
    ...
    rates_reindexed = list(arrival_rates)[start_plot_index:] + list(arrival_rates)[0:start_plot_index]
    labels_reindexed = list(hour_labels)[start_plot_index:] + list(hour_labels)[0:start_plot_index]
        ...
        for i, centile in enumerate(centiles):
            ...
            if centile == highlight_centile:
                for hour_line in hour_lines:
                    cumsum_at_hour = sum(percentiles[i][0:hour_line+1-start_plot_index])
    ...
    if not plot_centiles:
        if draw_window:
            ...
            plt.annotate(
                ...
                f'by {end_window}:00',
                xy=(hour_values[-1], y2 * .25),
                xytext=(hour_values[-1], y2 * .25),
                va='top',
                ha='right'
            )
            for hour_line in hour_lines:
