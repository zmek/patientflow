...
def plot_madcap_by_group(predict_proba, label, group, _prediction_time, group_name, media_path=None, plot_difference=True):
    ...
    fig.suptitle(f"MADCAP Plots by {group_name} for {hour}:{minutes:02}", fontsize=10, y = 1.04)
    ...
    if media_path:
        plot_name = f"madcap_plot_by_{group_name.replace(' ', '_')}_{hour}{minutes:02}.png"
