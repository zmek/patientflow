...
def plot_shap_individually(model_file_path, media_file_path, data_file_path, prediction_times, model_name):
    for i, _prediction_time in enumerate(prediction_times):
        fig, ax = plt.subplots(figsize=(8, 12))
        X_test, y_test, pipeline = prepare_for_inference(model_file_path = model_file_path, model_name = model_name, prediction_time = _prediction_time, data_path = data_file_path, single_snapshot_per_visit = False, model_only = False)
