...
def plot_trial_results(trials_list: List[HyperParameterTrial]):
    ...
    ax1.scatter([best_auroc_idx], [auroc_values[best_auroc_idx]], 
                color='green', s=150, edgecolor='black', zorder=5)
    
    # Highlight best Log Loss
    best_logloss_idx = logloss_values.index(min(logloss_values))
    ax2.scatter([best_logloss_idx], [logloss_values[best_logloss_idx]], 
                color='darkred', s=150, edgecolor='black', zorder=5)
    ...
    param_text_auroc = "\n".join([f"{k}: {v}" for k, v in best_auroc_trial.parameters.items()])
    ax1.text(0.05, 0.05, 
    ...
    param_text_logloss = "\n".join([f"{k}: {v}" for k, v in best_logloss_trial.parameters.items()])
    ax2.text(0.05, 0.05, 
