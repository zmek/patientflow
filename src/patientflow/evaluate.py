...
def predict_using_previous_weeks(df, dt, prediction_time, num_weeks):
    ...
    prediction_datetime = dt.replace(hour=prediction_time[0], minute=prediction_time[1]) 
    ...
    mask = (
        (df['arrival_datetime'] >= start_date) &
        (df['arrival_datetime'] < end_date) &
    ...
    target_date_count = len(df[(df['arrival_datetime'].dt.date == dt.date()) & 
...
def compare_six_week_average(prob_dist_dict_all: Dict[Any, Dict[Any, Dict[str, Any]]], 
                             df,
                             prediction_window,
                             x1, y1, x2, y2,
                            prediction_time, 
