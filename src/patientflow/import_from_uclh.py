...
def reformat_uclh_data_for_modelling(
    df,
    yta,
    start_training_set,
    ...
    encounter_mapping = {
        encounter: idx for idx, encounter in enumerate(df["encounter"].unique(), 1)
    }
    ...
    df = df[
        (df.arrival_datetime.dt.date >= start_training_set)
        & (df.arrival_datetime.dt.date <= end_test_set)
    ]
    yta = yta[
        (yta.arrival_datetime.dt.date >= start_training_set)
        & (yta.arrival_datetime.dt.date <= end_test_set)
    ]

    # assign each mrn to only the training, validation or test set
    df, yta = assign_mrn_to_training_validation_test_set(
        df,
        start_training_set,
        start_validation_set,
        start_test_set,
    ...
    df = df[
        (
            ...
            & (df.arrival_datetime.dt.date < start_validation_set)
        )
        | (
            (df.training_validation_test == "valid")
            & (df.arrival_datetime.dt.date >= start_validation_set)
            & (df.arrival_datetime.dt.date < start_test_set)
        )
        | (
    ...
    visits = prep_uclh_dataset_for_inference(
        df, uclh, remove_bed_requests, exclude_minority_categories, inference_time=False
    )

    merged_visits = (
        visits.merge(
            spec_mapping[["SubSpecialty", "Specialty"]].drop_duplicates(),
            how="left",
            left_on="specialty",
            right_on="SubSpecialty",
        )
        .drop(["SubSpecialty", "specialty"], axis=1)
        .rename(
            columns={
                "consultations": "consultation_sequence",
    ...
    merged_visits["consultation_sequence"] = merged_visits[
        "consultation_sequence"
    ].apply(lambda x: tuple(x) if x else ())
    merged_visits["final_sequence"] = merged_visits["final_sequence"].apply(
    ...
    yta = yta[
        ...
        | (
            ...
            & (yta.arrival_datetime.dt.date >= start_validation_set)
            & (yta.arrival_datetime.dt.date < start_test_set)
        )
        | (
    ...
    merged_yta = (
        ...
        .drop(["SubSpecialty", "specialty"], axis=1)
        .rename(columns={"Specialty": "specialty"})
    )

    merged_yta.index = yta.index
    merged_yta = merged_yta[
