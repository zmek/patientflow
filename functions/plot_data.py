...
def calculate_bins(data):
    ...
    if not np.isfinite(bins):
        return 10  # Fallback to a default number of bins if bins calculation is infinite
...
def plot_binned_histograms(df, col_name, group_by_col='is_admitted'):
    ...
    fig, axes = plt.subplots(nrows=len(unique_labels), ncols=1, figsize=(8, 3 * len(unique_labels)))  # Adjusted figsize
    for idx, label in enumerate(unique_labels):
        ...
        if group[col_name].dropna().empty:
            print(f"Skipping group {label}: column {col_name} contains only NaN values.")
            continue
        ax.hist(group[col_name].dropna(), bins=bins, edgecolor='black', color=color_palette[label])
...
def plot_categorical_histograms(df, col_name, group_by_col='is_admitted'):
    ...
    for idx, label in enumerate(unique_labels):
        ...
        if value_counts.empty:
            print(f"Skipping group {label}: column {col_name} contains only NaN values or no values.")
            continue
        value_counts.plot(kind='bar', ax=ax, color=color_palette[label])
        ax.set_title(f'{label} {group_by_col}')
        ax.set_xlabel(col_name)
        ax.set_ylabel('Count')
        ax.set_xticklabels(ax.get_xticklabels(), rotation=0)  # Ensure x-tick labels are not rotated
    plt.tight_layout()
    plt.show()

def plot_text_histograms(df, col_name, group_by_col='is_admitted', max_unique_values=20):
    df_copy = df.copy()
    ...
    value_counts_df.plot(kind='bar', ax=ax, color=[color_palette[label] for label in value_counts_df.columns])
...
def main_plot_function(df, exclude_from_plot, group_by_col='is_admitted'):
    for column in df.columns:
        if column not in exclude_from_plot:
            ...
            elif pd.api.types.is_numeric_dtype(df[column]) or pd.api.types.is_timedelta64_dtype(df[column]):
