...
class TestPrepareGroupSnapshotDict(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_data = pd.DataFrame({
            'snapshot_date': [
                ...
                date(2023, 1, 2), 
                date(2023, 1, 4), date(2023, 1, 4), date(2023, 1, 4),
                date(2023, 1, 7)
            ],
        ...
        self.end_date = date(2023, 1, 8)  # One day after last date to include full range
    ...
    def test_missing_column(self):
        ...
        self.assertTrue("DataFrame must include a 'snapshot_date' column" in str(context.exception))
    
    def test_start_date_only(self):
        """Test with only start_date provided (should use today as end_date)."""
        with patch('pandas.date_range') as mock_date_range:
            # Mock pd.date_range to test just the functionality
            mock_date_range.return_value = pd.DatetimeIndex([
                pd.Timestamp(self.start_date), 
                pd.Timestamp(self.start_date + timedelta(days=1))
            ])
            result = prepare_group_snapshot_dict(
                self.sample_data, 
                start_dt=self.start_date
            )
            # Verify pd.date_range was called with start and end
            mock_date_range.assert_called_once()
            args, kwargs = mock_date_range.call_args
            self.assertEqual(kwargs['start'], self.start_date)
            # The end date should be provided by the function, but we can't easily test the value
            self.assertIn('end', kwargs)
    
    def test_single_date(self):
        """Test with a DataFrame containing a single date."""
        single_date_df = pd.DataFrame({
            'snapshot_date': [date(2023, 1, 1)] * 3,
            'value': [10, 20, 30]
        })
